...
function applyHeaderLogo(url) {
  all(".configurable-mark").forEach((mark) => {
    const fallback = mark.querySelector(".mark-fallback");
    const image = mark.querySelector(".header-logo-image");
...
async function loadConfig() {
  try {
    ...
    const sharedHeroUrl = (config.heroLogoUrl || config.logoUrl || "").trim();
    const heroImageUrls = [
      config.heroImage1Url,
      config.heroImage2Url,
      config.heroImage3Url,
    ].map((url) => (url || sharedHeroUrl).trim());
    const headerLogoUrl = (config.headerLogoUrl || "").trim();
    all(".app-logo").forEach((image, index) => {
      const slot = Number(image.dataset.heroSlot || index);
      const imageUrl = heroImageUrls[slot] || sharedHeroUrl;
      if (imageUrl) image.src = imageUrl;
    });
    applyHeaderLogo(headerLogoUrl);
